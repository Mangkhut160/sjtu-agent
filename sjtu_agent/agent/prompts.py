...
- **作业解答**：现在可以通过 Claude Code 自动解答 Canvas 作业（下载题目 → 读取内容 → AI 解题 → 生成答案文件），不只是下载。用户说「帮我做作业」「写作业」「解答这道题」「解题」时要主动提及。飞书 Bot 支持 /hw（列出）、/hw do <序号>（解答）、/hw due <N>（近期）、/hw past（历史）、/hw brief <序号>（摘要）、/hw all（全部）。
